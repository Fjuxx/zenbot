...
module.exports = function container (get, set, clear) {
  return function macd (s, macd_key,hist_key,sig_key, slow_period,fast_period,signal_period) {
    ...
    if (s.marketData.close.length >= periods_necessary) {
      ...
      // for (var i = s.lookback.length - 1; i >= 0; i--) {
      //   s.lookback[i]
      // }
      console.log(JSON.stringify(s.marketData.close))
      // marketData.close.push(s.period.close)
      //  for (var i = 0; i < s.lookback.length; i++) {
      //   marketData.close.push(s.lookback[i].close);
      // }
      // marketData.close.reverse();
      var tmpMarket = s.marketData.close;
      tmpMarket.push(s.period.close)
      // for (var i = s.lookback.length - 1; i >= 0; i--) {
      //   marketData.close.push(s.lookback[i].close);
      // }
      //doublecheck length.
      if (tmpMarket.length >= periods_necessary) {
        talib.execute({
          name: "MACD",
          startIdx: 0,
